...
def main():
        ...
        if bdef['URI'] is not None:
            ...
            if bdef['hasDefiningText_fr'] is not None:
                g.add((element, PDF.hasDefiningText,
                       Literal(sf.formattxt(bdef['hasDefiningText_fr'],
                                            'latex'),
                               lang='fr')))
            if bdef['hasDefiningText_en'] is not None:
                g.add((element, PDF.hasDefiningText,
                       Literal(sf.formattxt(bdef['hasDefiningText_en'],
                                            'latex'),
                               lang='en')))
    ...
    with open(os.path.join(CUQ_FILES_FOLDER,
                           'si_units_special_names.yaml')) as fp:
    ...
    for sisp in si_spec_list:
        if sisp['URI'] is not None:
            ...
            g.add((element, PDF.hasUnitTypeAsString,
                   Literal('Unité SI dérivée ayant un nom spécial',
                           lang='fr')))
            g.add((element, SKOS.prefLabel,
    ...
    for nsi in non_si_list:
        if nsi['URI'] is not None:
            ...
            g.add((element, PDF.hasUnitTypeAsString,
                   Literal('Non-SI unit accepted for use with the SI',
                           lang='en')))
            g.add((element, PDF.hasUnitTypeAsString,
