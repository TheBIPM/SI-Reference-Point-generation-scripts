...
class SiElements:
    ...
    # 1) Define namespaces, sub-namespace and shortcuts used by A boxes
       ...
       # Define the namespaces within (base)/bodies
        ...
        for ttl_file in ['si_core_concepts.ttl',
                         'si_extended_concepts.ttl']:
            self.g.parse(os.path.join(SI_FILES_FOLDER, ttl_file),
                         format="ttl")
...
### `generate_turtle_files`
    ...
    #     and define the URI to a specific version by using its commit on GitHub
        agent_sw = GITHUB_BASE_PATH +"blob/"+ sha + "/src/si_ref_point/tboxes/si_tbox.py"
        self.g.add(
            (URIRef(agent_sw),
             RDF.type,
             PROV.Agent)
