...
Apart from allogin some testing this also helps to backport additional
...
def get_unit(g, obj):
    if not obj.isidentifier():
        ...
        if rdftype == URIRef(si + 'UnitProduct'):
            for s, p, o in list(g.triples((obj, URIRef(si + 'hasLeftUnitTerm'), None))):
                left_term = get_unit(g, o)
            for s, p, o in list(g.triples((obj, URIRef(si + 'hasRightUnitTerm'), None))):
                right_term = get_unit(g, o)
            return [left_term, right_term]
        elif rdftype == URIRef(si + 'UnitPower'):
            num_ex = None
            for s, p, o in list(g.triples((obj, URIRef(si + 'hasNumericExponent'), None))):
                num_ex = o.value
            unit = None
            for s, p, o in list(g.triples((obj, URIRef(si + 'hasUnitBase'), None))):
                unit = get_unit(g, o)
                unit[1] = num_ex
            return unit
        else:
...
def main():
    ...
    qty_list = []
    for s, p, o in list(g.triples((None, URIRef(rdf + 'type'), URIRef(si + "QuantityKind")))):
        q_id = str(s).split('/')[-1]
