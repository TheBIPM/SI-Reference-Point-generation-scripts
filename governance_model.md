...
Before opening a new issue, it is desirable to look for an already opened issue that already relates to the topic.
...
View-only access can be granted to members of the TG-SIDF upon request (a free GitHub account is required).
...
Depending on the prioritization and complexity level of the issue, one or more Expert Group members may volunteer to work on the code implementation depending on their availability and area of expertise.
...
---
#### Consultation
This status is assigned to requests from the [<ins>Backlog</ins>](#backlog) which it was considered must be discussed at an instance higher than the Expert Group because it implies a high level decision, or because there is strong disagreement among the Expert Group members regarding the suitability of the request.
...
The default instance to consider for consultation is the TG-SIDF, but the Expert Group members can also decide to discuss the topic directly with another working group of the FORUM-MD, the Consultative Committee of Units (CCU), or another Consultative Committee of the CIPM.
...
Depending on the outcome of the review, the issue can be moved back to status [<ins>In progress</ins>](#in-progress), or advanced to status [<ins>Awaiting release</ins>](#awaiting-release).
...
---
#### Not planned
This status is assigned for issues from the boxes [<ins>Backlog</ins>](#backlog) or [<ins>Consultation</ins>](#consultation), when the corresponding decision body has considered them unsuitable under current circumstances.
The person that orginally made the request is notified about the decision of not proceeding with the request, with a summary of the reasons considered.
...
When the code patch that solves a request is considered finished by the people involved, they shall request to merge the changes from the feature branch (in a forked repository in the case of external contributors), into the branch `origin/develop`.
...
This assessment should consider potential effects on other BIPM digital services that rely on the SIRP, like the Key Comparison Database (KCDB).
...
After confirming that the changes maintain backward compatibility,
 ...
 >*Here we need to decide on one of the following approaches*
...
9.  A successful release is indicated to the community by using the BIPM communications channels, as considered appropriate by the TG-SIDF chair.
10. The people whose requests were satisfied with the release of the SIRP are notified.
